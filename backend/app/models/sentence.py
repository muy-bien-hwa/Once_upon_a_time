...
class Sentence(IdMixin, CreatedAtMixin, Base): # 파이썬 클래스 1개 = 테이블 1개. id·created_at은 공통 부품에서 받아옴
    __tablename__ = "sentences"   # DB에 만들어질 테이블 이름
    __table_args__ = (   # 테이블 전체에 거는 규칙
        CheckConstraint(r"content ~ '\S'", name="content_not_blank"),   # 공백만 있는 문장 거부
        CheckConstraint("depth >= 0", name="depth_non_negative"),
        CheckConstraint("(parent_id IS NULL) = (depth = 0)", name="root_iff_depth_zero"),   # 부모 없음 ⇔ 깊이 0
    ...
    story_id: Mapped[int] = mapped_column(BigInteger, ForeignKey("stories.id"))   # 필수 + stories 테이블 참조
    parent_id: Mapped[int | None] = mapped_column(BigInteger, ForeignKey("sentences.id"))   # `| None` = 비어도 됨 (첫 문장)
    ...
    status: Mapped[str] = mapped_column(String(10), server_default="active")   # 값을 안 주면 'active'
